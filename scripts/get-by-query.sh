#!/bin/sh
export GET_BY_QUERY=$(grep -r Db.getByQuery * --exclude=*.class | grep -v WithParams | wc -l)
export GET_UNIQUE_BY_QUERY=$(grep -r Db.getUniqueByQuery * --exclude=*.class | grep -v WithParams | wc -l)
